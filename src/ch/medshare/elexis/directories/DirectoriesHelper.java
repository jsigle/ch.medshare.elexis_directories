...
public class DirectoriesHelper {
	...
	private static String cleanupText(String text){
		//20120712 some additions that remove definitely unwanted portions of the returned html code:
		//text.replace							uses search strings literally
		//text.replaceFirst, text.replaceAll	use regex search strings

		//Remove extra boxes, maps etc. at the beginning - everything between </title> and <div class='summary'>
		...
		//I add a comment to warn you if you sould desire to use XPath or something like that in the future. 
		...
		//Remove the complete footer of the page. (outdated on the same day - I need to remove even more - see coment below)
		//?s enables "single line" mode, so that the search extends over multiple lines:
		//Please note that this may break the HTML tree hierarchy;
		//I add a comment to warn you if you sould desire to use XPath or something like that in the future. 
		...
		//This may also break the tree structure, but as of 20120712, it might not even exist in the incoming html code any more. js 
	...
	private static String cleanupUmlaute(String text){
		/*
		text = text.replace("&#xE4;", "ae");//$NON-NLS-1$ //$NON-NLS-2$
		text = text.replace("&#xC4;", "Ae");//$NON-NLS-1$ //$NON-NLS-2$
		text = text.replace("&#xF6;", "oe");//$NON-NLS-1$ //$NON-NLS-2$
		text = text.replace("&#xD6;", "Oe");//$NON-NLS-1$ //$NON-NLS-2$
		text = text.replace("&#xFC;", "ue");//$NON-NLS-1$ //$NON-NLS-2$
		text = text.replace("&#xDC;", "Ue");//$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * 20120713js:
	 * 
	 *          Currently, requests to both tel.local.ch return a page that appears to be formatted for WAP,
	 *          and includes references to http://mobile.local.c/... when obtained through the elexis plugin.
	 *          The same url returns a proper www page when obtained through mozilla firefox.
	 *          The problematic part of this is that I cannot see the desired results in the wap version :-(
	 *          So I try to set a user agent as well...
	 *           
	 *          Ein &range=all könnte eventuell alle Ergebnisse zurückliefern - 
	 *          nein, das tut es nicht (oder: derzeit nicht *mehr*),
	 *          genauso scheint auch &mode=text ignoriert zu werden.
	 */
	...
	/**
	 * Schreibt binäre Datei 20101213js: Ich bin nicht sicher, ob das überhaupt verwendet wird?
	 ...
	 *          complete html page returned) for a search request to tel.local.ch constructed from name and geo.
	 *          
	 * 20120713js:
	 * 
	 *          Apparently, requests to both tel.local.ch would now return a page formatted for WAP,
	 *          and including references to http://mobile.local.c/... when obtained through the elexis plugin.
	 *          
	 *          The same url would, however, return a proper www page when obtained through mozilla firefox or wget.
	 *          The problematic point is: I cannot see the desired results in the wap version :-(
	 *          (Even though they appear when the download is displayed in a browser again,
	 *           maybe through some javascript construct, or I just missed something.)
	 *          Anyway, the ContentParser of this plugin also does not see anything of interest in the WAP page it receives.
	 *          So I want to set user agent for the request to tel.local.ch
	 *          
	 *          In order to set a user agent, I need to use a different approach to obtain http content,
	 *          which uses a connection. And yes - as long as I set any userAgent string - even if it's empty -
	 *          that works and I get a real WWW page including the desired content.
	 *            
	 ...
	 *          namely everything between ...</title> and <div class='summary'>...,
	 *          before returning the result. This has been added to cleanupText() above.
	 *          
	 *          It's not really beautiful code that does reading and preprocessing in the same method.
	 *          But after all, obfuscating some 10 addresses in 45KB of javascript/html is not beautiful either.
	 */
	public static String readContent(final String name, final String geo) throws IOException,
		 ...
		 * Original code:
		 * 
		 * This code did not set a user agent string.
		 * And as it did not explicitly use a connection, I think we couldn't make it do either.
		 * 		
		...
		/*
		 * New code by js 20120712
		 * based upon: http://jawe.net/2006/07/23/setting-the-http-user-agent-in-java/
		 */
		
		System.out.print("jsdebug: DirectoriesHelper.java readContent() running...\n");

		//It is apparently NOT necessary to change our user agent to Mozilla to avoid getting a wap page.
		//Switching over to the connection based URL reading approach AND setting ANY User-Agent AT ALL (!) 
		...
		String userAgent = "Elexis/js www.jsigle.com/prog/elexis";	//receives: <div class='container'>
